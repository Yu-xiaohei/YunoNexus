...
function SystemSettings() {
  ...
  useEffect(() => {
    const savedGeneral = localStorage.getItem('yunonexus_general')
    const savedSecurity = localStorage.getItem('yunonexus_security')
    const savedUserDefault = localStorage.getItem('yunonexus_user_defaults')
    
    if (savedGeneral) generalForm.setFieldsValue(JSON.parse(savedGeneral))
    if (savedSecurity) securityForm.setFieldsValue(JSON.parse(savedSecurity))
    if (savedUserDefault) userDefaultForm.setFieldsValue(JSON.parse(savedUserDefault))
  }, [])
  ...
  const tabItems = [
    ...
    },
    {
      ...
      label: '用户默认设置',
      children: (
        <Form form={userDefaultForm} layout="vertical" style={{ maxWidth: 600 }}>
          <Alert 
            message="提示" 
...
function PortPolicyForm() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('yunonexus_port_rules')
    if (saved) setRules(JSON.parse(saved))
    const savedTags = localStorage.getItem('yunonexus_port_tag_types')
    if (savedTags) setTagTypes(JSON.parse(savedTags))
  }, [])

  const handleSaveRules = () => {
    localStorage.setItem('yunonexus_port_rules', JSON.stringify(rules))
    localStorage.setItem('yunonexus_port_tag_types', JSON.stringify(tagTypes))
    message.success('端口策略保存成功')
  }

  const handleAddRule = () => {
    if (!newRule.value) { message.warning('请输入端口或端口范围'); return }
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <Alert message="端口策略配置" description="配置隧道可用的端口规则。" type="info" showIcon style={{ marginBottom: 16 }} />
      <Space direction="vertical" style={{ width: '100%' }}>
        <Space wrap>
          <select value={newRule.type} onChange={(e) => setNewRule({...newRule, type: e.target.value})} style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid #d9d9d9' }}>
            <option value="single">单个端口</option>
            <option value="range">端口区间</option>
          </select>
          <Input placeholder={newRule.type === 'single' ? '例如: 8080' : '例如: 30000-30100'} value={newRule.value} onChange={(e) => setNewRule({...newRule, value: e.target.value})} style={{ width: 200 }} />
          <select value={newRule.tag} onChange={(e) => setNewRule({...newRule, tag: e.target.value})} style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid #d9d9d9' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 16 }}>
          ...
          <tbody>
            ...
            {rules.length === 0 && <tr><td colSpan={4} style={{ padding: '8px', textAlign: 'center', color: '#999', border: '1px solid #e8e8e8' }}>暂无规则</td></tr>}
          </tbody>
        </table>
      </Space>
      {showTagModal && (
        <div style={{ marginTop: 16, padding: 16, background: '#f5f5f5', borderRadius: 8 }}>
          <h4>新建标签类型</h4>
          <Space>
            <Input placeholder="输入标签名称" value={newTagType} onChange={(e) => setNewTagType(e.target.value)} style={{ width: 200 }} />
            <Button type="primary" onClick={handleAddTagType}>添加</Button>
            <Button onClick={() => setShowTagModal(false)}>取消</Button>
          </Space>
          <div style={{ marginTop: 8 }}><span>已有标签: </span>{tagTypes.map(tag => <span key={tag} style={{ background: '#e6f7ff', padding: '2px 8px', borderRadius: 4, marginRight: 8 }}>{tag}</span>)}</div>
        </div>
      )}
      <Form.Item style={{ marginTop: 16 }}><Button type="primary" onClick={handleSaveRules}>保存端口策略</Button></Form.Item>
    </div>
  )
}
...
function PasswordPolicyForm() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('yunonexus_password_policy')
  ...
  return (
    <Form form={form} layout="vertical" style={{ maxWidth: 600 }}>
      <Alert message="密码策略配置" description="设置新用户的密码强度要求。" type="info" showIcon style={{ marginBottom: 16 }} />
      <Form.Item name="min_length" label="最小密码长度"><InputNumber defaultValue={8} min={6} max={32} style={{ width: '100%' }} /></Form.Item>
      <Form.Item name="require_uppercase" label="必须包含大写字母" valuePropName="checked"><Switch defaultChecked /></Form.Item>
      <Form.Item name="require_lowercase" label="必须包含小写字母" valuePropName="checked"><Switch defaultChecked /></Form.Item>
      <Form.Item name="require_digit" label="必须包含数字" valuePropName="checked"><Switch defaultChecked /></Form.Item>
      <Form.Item name="require_special" label="必须包含特殊字符" valuePropName="checked"><Switch defaultChecked /></Form.Item>
      <Form.Item><Button type="primary" onClick={handleSave} loading={saving}>保存</Button></Form.Item>
    </Form>
  )
}
